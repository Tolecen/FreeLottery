...
@synthesize globalView =_globalView;
...
- (id)initWithTitle:(NSString *)title 
{
    if ((self = [super init]))
    {
        ...
        _globalView = [[UIView alloc] initWithFrame:frame];
        
        _view.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleTopMargin;
        _globalView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleTopMargin;
...
#pragma clan diagnostic pop
    ...
    //-----------------模态空白位置点击效果----------------------------------------------
    UIButton *emptyBtnView = [UIButton buttonWithType:UIButtonTypeCustom];
    emptyBtnView.frame = CGRectMake(0, 0, 320, [BlockBackground sharedInstance].bounds.size.height - (_height + kActionSheetBounce));

    emptyBtnView.backgroundColor = [UIColor clearColor];
    
    [emptyBtnView addTarget:self action:@selector(buttonClicked:) forControlEvents:UIControlEventTouchUpInside];
    
    emptyBtnView.autoresizingMask = UIViewAutoresizingFlexibleWidth;
    
    [_globalView addSubview:emptyBtnView];
    
    //-------------------------------------------------------------------------
    
    
    [_globalView addSubview:_view];
    
    [BlockBackground sharedInstance].vignetteBackground = _vignetteBackground;
    [[BlockBackground sharedInstance] addToMainWindow:_globalView];
    ...
    if (animated)
    {
        ...
        [UIView animateWithDuration:0.4
                         ...
                         animations:^{
                             ...
                             [[BlockBackground sharedInstance] removeView:_globalView];
                             [_view release]; _view = nil;
                             [_globalView release]; _globalView = nil;
                             [self autorelease];
                         }];
    }
    else
