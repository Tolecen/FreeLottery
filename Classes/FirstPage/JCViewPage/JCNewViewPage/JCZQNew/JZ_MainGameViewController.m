...
@interface JZ_MainGameViewController (internal)
...
@implementation JZ_MainGameViewController
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"rightDrawerView" object:nil];
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"leftDrawerView" object:nil];
    ...
    [[NSUserDefaults standardUserDefaults] setObject:[NSString stringWithFormat:@"%d",m_playTypeTag] forKey:@ "playTypeTag" ];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(rightDrawerView:) name:@"rightDrawerView" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(leftDrawerView:) name:@"leftDrawerView" object:nil];
    ...
    [iamge release];
    //余额不足页面返回前页面的时候回出现中间按钮和精彩投注变体同时出现，需要进行以下处理
    if (m_jz_NoteNumEditeViewController.isScrollerLeft)
    {
        self.navigationItem.title = @"竞彩足球投注";
...
- (void)viewDidLoad {
    ...
    NSString *playTagStr = [[NSUserDefaults standardUserDefaults] objectForKey:@ "playTypeTag"];
    
    
    if (playTagStr)
    {
        m_playTypeTag = [playTagStr intValue];
    }else
    {
    ...
    m_bottomScrollView.backgroundColor = [ColorUtils parseColorFromRGB:@"#d9d4cb"];
    
    [m_buttonBuy setBackgroundColor:[ColorUtils parseColorFromRGB:@"#d52322"]];
    [m_buttonReselect setBackgroundColor:[ColorUtils parseColorFromRGB:@"#60513b"]];
...
- (void)creatBetVC
{
    m_jz_NoteNumEditeViewController = [[JZ_NoteNumEditeViewController alloc]init];
    m_jz_NoteNumEditeViewController.view.backgroundColor = [UIColor clearColor];
    m_jz_NoteNumEditeViewController.view.frame = CGRectMake(0,0, 340, [UIScreen mainScreen].bounds.size.height);
    [m_jz_NoteNumEditeViewController.editeButton addTarget:self action:@selector(editeButtonClick) forControlEvents:UIControlEventTouchUpInside];
    
    [m_jz_NoteNumEditeViewController.editeButton addTarget:self action:@selector(buttonTouchDown) forControlEvents:UIControlEventTouchDown];
    
    [m_jz_NoteNumEditeViewController.editeButton addTarget:self action:@selector(buttonTouchUp) forControlEvents:UIControlEventTouchCancel];
    
    UIImageView *leftDragImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"right_drawer.png"]];
    leftDragImageView.frame = CGRectMake(20, 150, 19, 34);
    [m_jz_NoteNumEditeViewController.view addSubview:leftDragImageView];
    [leftDragImageView release];
    
    m_drawerView = [[DrawerView alloc]initWithView:m_jz_NoteNumEditeViewController parentView:self.view];
    m_drawerView.backgroundColor = [UIColor clearColor];
    
    [self.view insertSubview:m_drawerView aboveSubview:m_tableView];
    [m_drawerView.countEventLable setHidden:YES];
    [m_drawerView.countUIImageView setHidden:YES];
}

- (void)editeButtonClick
{
    m_numGameCount = [m_jz_NoteNumEditeViewController.dataBaseArray count];
    m_jz_NoteNumEditeViewController.isScrollerLeft = NO;
    [self numGameCountChange];
    [m_tableView  reloadData];
    
    
    //滑回显示页面
    [self setDrawerCenter:CGPointMake((-20+self.view.frame.size.width)/2+320, self.view.frame.size.height-[UIScreen mainScreen].bounds.size.height/2-40+44)];
    m_jz_NoteNumEditeViewController.editeButton.backgroundColor = [ColorUtils parseColorFromRGB:@"#e6e0d7"];
    self.navigationItem.title = nil;
    [centerButton setHidden:NO];
    [m_detailButton setHidden:NO];
}
...
- (void)numGameCountChange
{
    // 已选比赛：%d 场
    if (m_playTypeTag == IJCZQPlayType_RQ_SPF ||m_playTypeTag == IJCZQPlayType_SPF || m_playTypeTag == IJCZQPlayType_RQANDNO_SPF) {
        m_totalCost.text = [NSString stringWithFormat:@"%d 场\n", m_numGameCount];
        
    }
    else
    {
        m_totalCost.text = [NSString stringWithFormat:@"%d 场", m_numGameCount];
    }
    if (m_numGameCount>0)
    {
        [m_drawerView.countUIImageView setHidden:NO];
        [m_drawerView.countEventLable setHidden:NO];
        
        m_drawerView.countEventLable.text = [NSString stringWithFormat:@"%d",m_numGameCount];
    }else
    {
        [m_drawerView.countEventLable setHidden:YES];
        [m_drawerView.countUIImageView setHidden:YES];
...
//点击滑动效果动画
- (void)setDrawerCenter:(CGPoint)center
{
    [UIView beginAnimations:nil context:nil];
    [UIView setAnimationCurve:UIViewAnimationCurveEaseIn];
    [UIView setAnimationDuration:0.5f];
    [UIView setAnimationRepeatAutoreverses:NO];
    m_drawerView.center = center; 
    self.navigationItem.title = nil;
    [UIView commitAnimations];
}
//这两个方法的作用为按钮做效果的，因为按钮没背景图片，使用的颜色；
- (void)buttonTouchDown
{
    m_jz_NoteNumEditeViewController.editeButton.backgroundColor = [UIColor grayColor];
}
- (void)buttonTouchUp
{
    m_jz_NoteNumEditeViewController.editeButton.backgroundColor = [ColorUtils parseColorFromRGB:@"#e6e0d7"];
...
//判断进入该页面应该算中那个按钮虽然第一次进入选中胜负平，但是按钮不会自己选中所以需要代码进行控制
- (void)judgeSelectButon
{
    if (m_playTypeTag == IJCZQPlayType_SPF || m_playTypeTag == IJCZQPlayType_RQANDNO_SPF)
    {
        [m_playChooseView thatButtonSelect:1];
    }else if(m_playTypeTag == IJCZQPlayType_ZJQ)
    {
        [m_playChooseView thatButtonSelect:2];
    }
    else if(m_playTypeTag == IJCZQPlayType_HalfAndAll)
    {
        [m_playChooseView thatButtonSelect:3];
    }
    else if(m_playTypeTag == IJCZQPlayType_Score)
    {
        [m_playChooseView thatButtonSelect:4];
    }
    else
    {
        [m_playChooseView thatButtonSelect:5];
    }
...
- (void)pressTitle:(id)sender
{
    ...
    if ([m_tableHeaderState objectAtIndex:temptag] == @"0")
    {
        ...
        m_SectionN[temptag] = [(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:temptag] baseCount];
        
        [temp setBackgroundImage:[UIImage imageNamed:@"jc_section_expand.png"] forState:UIControlStateNormal];
        [temp setBackgroundImage:[UIImage imageNamed:@"jc_section_hide.png"] forState:UIControlStateHighlighted];
    }
        ...
        [temp setBackgroundImage:[UIImage imageNamed:@"jc_section_hide.png"] forState:UIControlStateNormal];
        [temp setBackgroundImage:[UIImage imageNamed:@"jc_section_expand.png"] forState:UIControlStateHighlighted];
    }
...
{
    JZTableViewCell_DataBase*  base = (JZTableViewCell_DataBase*)[[(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:indexPath.section] tableHeaderArray] objectAtIndex:indexPath.row];
    if (m_playTypeTag == IJCZQPlayType_RQANDNO_SPF)
    {
        if (base.isExpend)
        {
            //  return 127;
...
- (UIView*) tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    else
    {
        ...
        [button setBackgroundColor:[ColorUtils parseColorFromRGB:@"#e8e4dd"]];
        ...
        if ([m_tableHeaderState objectAtIndex:tempSection] == @"0"){
            UIImageView *image = [[UIImageView alloc] initWithFrame:CGRectMake(295, 5, 17, 17)];
            image.image = [UIImage imageNamed:@"jc_section_hide.png"];
            [button addSubview:image];
            [image release];
        }
        else if([m_tableHeaderState objectAtIndex:tempSection] == @"1"){
            UIImageView *image = [[UIImageView alloc] initWithFrame:CGRectMake(295, 5, 17, 17)];
            image.image = [UIImage imageNamed:@"jc_section_expand.png"];
            [button addSubview:image];
            [image release];
        }
        ...
        NSMutableArray* tempArray = [[(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:tempSection] tableHeaderArray] objectAtIndex:0];//获取数组中的 第一个base数据
        
        NSString* dayForamt = [NSString stringWithFormat:@"%@  ", [(JZTableViewCell_DataBase*) tempArray dayforamt]];
        NSString* week = [NSString stringWithFormat:@"%@  ",[(JZTableViewCell_DataBase*) tempArray week]];
        NSString* title = [NSString stringWithFormat:@"%@%@%d场比赛可投",dayForamt,week,[((JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:tempSection]) baseCount]];
...
{
    UIView *viewFootTable  = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 320, 1)];
    viewFootTable.backgroundColor = [ColorUtils parseColorFromRGB:@"#d9d6d1"];
    return [viewFootTable autorelease];
}
...
- (UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (m_playTypeTag == IJCZQPlayType_RQANDNO_SPF)
    {
        ...
        JZHeighTableViewCell *cell = (JZHeighTableViewCell*)[tableView dequeueReusableCellWithIdentifier:groupCell];
        if(cell == nil)
        {
            cell = [[[JZHeighTableViewCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:groupCell]autorelease];
            
            [cell.expendButton addTarget:self action:@selector(expendButtonClick:) forControlEvents:UIControlEventTouchUpInside];
        }
        ...
        JZTableViewCell_DataBase* base;
        
        if ([m_motionArray count] > 0 && indexPath.section == 0) {
            
            base = (JZTableViewCell_DataBase*)[[(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:[[m_motionArray objectAtIndex:(indexPath.row == 0)? 0 : 2] intValue]] tableHeaderArray] objectAtIndex:[[m_motionArray objectAtIndex:(indexPath.row == 0)? 1 : 3] intValue]];
            ...
            cell.isRQLeftSelect  = [base RQZQ_S_ButtonIsSelect];
            cell.isRQCenterSelect= [base RQZQ_P_ButtonIsSelect];
            cell.isRQRightSelect = [base RQZQ_F_ButtonIsSelect];
        ...
        else
        {
             ...
             */
            base = (JZTableViewCell_DataBase*)[[(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:section] tableHeaderArray] objectAtIndex:indexPath.row];
            
            cell.playTypeTag = m_playTypeTag;
                
                //删除的选中置为为选中
                if (m_jz_NoteNumEditeViewController.deleteBaseArray &&[m_jz_NoteNumEditeViewController.deleteBaseArray count]!=0)
                {
                    for (JZTableViewCell_DataBase* key in m_jz_NoteNumEditeViewController.deleteBaseArray)
                    {
                        if (key ==base)
                        {
                            ...
                            [base setRQZQ_S_ButtonIsSelect:NO];
                            [base setRQZQ_P_ButtonIsSelect:NO];
                            [base setRQZQ_F_ButtonIsSelect:NO];
                        }
        ...
        cell.endTime = [self StringFormatterString:[base endTime]];
    ...
    {
        static NSString *groupCell = @"groupallCell";
        JZTableViewCell *cell = (JZTableViewCell*)[tableView dequeueReusableCellWithIdentifier:groupCell];
        if(cell == nil)
        {
            cell = [[[JZTableViewCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:groupCell]autorelease];
...
//cell的伸缩功能
- (void)expendButtonClick:(id)sender
{
    UIButton *expendButton = (UIButton *)sender;
       JZHeighTableViewCell *cell = (JZHeighTableViewCell*)[expendButton superview];
        NSIndexPath *indexpath = [m_tableView indexPathForCell:cell];
        NSArray *updatapaths = [NSArray arrayWithObject:indexpath];
        
        JZTableViewCell_DataBase*  base = [[(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:indexpath.section] tableHeaderArray] objectAtIndex:indexpath.row];
        
        base.isExpend=!base.isExpend;
        [m_tableView reloadRowsAtIndexPaths:updatapaths withRowAnimation:NO];
    
}
//时间字符串转时间之后在转化自己要的类型的时间字符串
- (NSString *)StringFormatterString:(NSString *)dateString
{
    NSString *strby = [dateString stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
    NSDateFormatter *inputFormatter = [[[NSDateFormatter alloc] init] autorelease];
    [inputFormatter setLocale:[[[NSLocale alloc] initWithLocaleIdentifier:@"en_US"] autorelease]];
    [inputFormatter setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
    NSDate* inputDate = [inputFormatter dateFromString:strby];
    NSLog(@"date = %@", inputDate);
    
    NSDateFormatter *outputFormatter = [[[NSDateFormatter alloc] init] autorelease];
    [outputFormatter setLocale:[NSLocale currentLocale]];
    [outputFormatter setDateFormat:@"HH:mm"];
    NSString *currentDateStr = [outputFormatter stringFromDate:inputDate];
...
- (void)submitLotNotification:(NSNotification*)notification
{
    ...
    {
        int everyHeaderBaseCount = [(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:i] baseCount];
        for (int j = 0; j < everyHeaderBaseCount; j++) {
             ...
             */
            JZTableViewCell_DataBase* base = (JZTableViewCell_DataBase*)[[(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:i] tableHeaderArray] objectAtIndex:j];
            ...
            if (self.playTypeTag == IJCZQPlayType_Confusion)
            {
                ...
                //每一个对阵赔率
                JZ_CombineBase *gameSparray = [[[JZ_CombineBase alloc] init] autorelease];
                ...
                if (ishave) {
                    ...
                    //保存Event
                    [m_eventChooseGameArray addObject:base];
            ...
            else
            {
                    ...
                    m_playTypeTag == IJCZQPlayType_Score_DanGuan)
                {
                    ...
                    //每一个对阵赔率
                    JZ_CombineBase *gameSparray = [[JZ_CombineBase alloc] init];
	...
	NSLog(@"betCode******1 %@",betCode);
    NSLog(@"betCode******2 %@",disBetCode);
    NSLog(@"betCode******3 %@",betCode_Dan);
    ...
    NSLog(@"betCode******4 %@",betCode_dan_tuo);
...
- (void)betNormal:(NSNotification*)notification
{
    ...
    //销毁duoChuanChooseArray 不然每次投注会叠加
    m_jz_NoteNumEditeViewController.duoChuanChooseArray = nil;
    
    [m_jz_NoteNumEditeViewController initAppendArrangePS];
    for (int spindex = 0; spindex < [m_arrangeSP count]; spindex++)
    {
        JZ_CombineBase* array = (JZ_CombineBase*)[m_arrangeSP objectAtIndex:spindex];
        
        [m_jz_NoteNumEditeViewController appendArrangePS:array];
        NSLog(@"array %@", array.combineBase_SP);
    }
    //排序
    [m_jz_NoteNumEditeViewController sortSPArray];
        ...
        )
    {
        ...
        for (int i = 0; i < m_headerCount; i++)
        {
            int baseCount = [(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:i] baseCount];
    ...
    if (m_playTypeTag == IJCZQPlayType_RQ_SPF ||
        ...
        m_playTypeTag == IJCZQPlayType_HalfAndAll||
        m_playTypeTag == IJCZQPlayType_Confusion||
        m_playTypeTag == IJCZQPlayType_RQANDNO_SPF)
    {
        for (int i = 0; i < m_headerCount; i++)
        {
            int baseCount = [(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:i] baseCount];
            for (int j = 0; j < baseCount; j++)
            {
                JZTableViewCell_DataBase* base = [[(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:i] tableHeaderArray] objectAtIndex:j];
                if (m_playTypeTag == IJCZQPlayType_Confusion)
                {
                    //计算 选择的
                        ...
                        if ([array_inner count] > 0)
                        {
                            ...
                            else if (m == 2)//竞彩足球半全场
                            {
                                ...
                                for (int inner_index = 0; inner_index < [array_inner count]; inner_index++) {
                                    ...
                                    m_jz_NoteNumEditeViewController.confusion_type = JCZQ_HALF;
                                }
                                m_playTypeTag = IJCZQPlayType_Confusion;
                            }
                            else if (m == 3)//竞彩足球总进球
                            {
                                for (int inner_index = 0; inner_index < [array_inner count]; inner_index++) {
                                    ...
                                    if (m_jz_NoteNumEditeViewController.confusion_type != JCZQ_HALF && m_jz_NoteNumEditeViewController.confusion_type != JCZQ_SCORE) {
                                        m_jz_NoteNumEditeViewController.confusion_type = JCZQ_ZJQ;
                                    }
                                }
                            }
                            else if (m == 4)//竞彩足球比分
                            {
                                ...
                                for (int inner_index = 0; inner_index < [array_inner count]; inner_index++) {
                                    ...
                                    m_jz_NoteNumEditeViewController.confusion_type = JCZQ_SCORE;
                                }
                                m_playTypeTag = IJCZQPlayType_Confusion;
                            }
                        }
                    ...
                    if (confusion_selectCount > 0) {
                        [m_jz_NoteNumEditeViewController appendDuoChuanChoose:[NSString stringWithFormat:@"%d",confusion_selectCount] IS_DAN:[base JC_DanIsSelect] CONFUSION:confusion_array];
                    }
                    //////////////// //////// //////// ////////
                ...
                else
                {
                    ...
                    if (m_playTypeTag == IJCZQPlayType_RQ_SPF ||m_playTypeTag == IJCZQPlayType_SPF)
                    {
                        ...
                        if (ZQ_S_ButtonIsSelect || ZQ_P_ButtonIsSelect || ZQ_F_ButtonIsSelect)
                        {
                               ...
                               )//选2个或3个结果时
                            {
                                if (ZQ_S_ButtonIsSelect && ZQ_P_ButtonIsSelect && ZQ_F_ButtonIsSelect)
                                {
                                    [m_jz_NoteNumEditeViewController appendDuoChuanChoose:@"3" IS_DAN:[base JC_DanIsSelect] CONFUSION:nil];
                                }
                                else
                                    [m_jz_NoteNumEditeViewController appendDuoChuanChoose:@"2" IS_DAN:[base JC_DanIsSelect] CONFUSION:nil];
                            }
                            else//选1个结果时
                            {
                                [m_jz_NoteNumEditeViewController appendDuoChuanChoose:@"1" IS_DAN:[base JC_DanIsSelect] CONFUSION:nil];
                            }
                        }
                    }
                    else
                    {
                        if (selectCount > 0)
                        {
                            ...
                            [m_jz_NoteNumEditeViewController appendDuoChuanChoose:temp IS_DAN:[base JC_DanIsSelect] CONFUSION:nil];
                        }
                    }
                }
            }
    ...
    [m_jz_NoteNumEditeViewController.freePassRadioIndexArray removeAllObjects];
    if ([m_jz_NoteNumEditeViewController.allUpScrollView subviews]) {
        for (UIView *keyView in [m_jz_NoteNumEditeViewController.allUpScrollView subviews]) {
    ...
    m_jz_NoteNumEditeViewController.twoCount = m_twoCount;
    m_jz_NoteNumEditeViewController.dataBaseArray = m_eventChooseGameArray;
    m_jz_NoteNumEditeViewController.numGameCount = [m_jz_NoteNumEditeViewController.dataBaseArray count];
    m_jz_NoteNumEditeViewController.chooseBetCode = [RuYiCaiLotDetail sharedObject].betCode;

    m_jz_NoteNumEditeViewController.playTypeTag = self.playTypeTag;
//   下面就是刷新投注确认页面的表和底部view
    [m_jz_NoteNumEditeViewController.tableView reloadData];

    [m_jz_NoteNumEditeViewController changeBetNumber];
    [m_jz_NoteNumEditeViewController setFreePassScollView];
    [m_jz_NoteNumEditeViewController setDuoChuanPassScollView];
    m_jz_NoteNumEditeViewController.isScrollerLeft = YES;
    [self setDrawerCenter:CGPointMake((-20+self.view.frame.size.width)/2, self.view.frame.size.height-[UIScreen mainScreen].bounds.size.height/2-40+44)];
    [m_detailButton setHidden:YES];
    [centerButton setHidden:YES];
    self.navigationItem.title = @"竞彩足球投注";
...
- (void)rightDrawerView:(NSNotification*)notification

{
    m_numGameCount = [m_jz_NoteNumEditeViewController.dataBaseArray count];
    m_jz_NoteNumEditeViewController.isScrollerLeft = NO;
    NSLog(@"rightDrawerViewrightDrawerView");
    [centerButton setHidden:NO];
    self.navigationItem.title = nil;
    [m_detailButton setHidden:NO];
    [self numGameCountChange];
...
- (void)leftDrawerView:(NSNotification*)notification

{
    ...
    [centerButton setHidden:YES];
    self.navigationItem.title = @"竞彩足球投注";
    NSLog(@"leftDrawerViewleftDrawerView");
    [self submitLotNotification:nil];
}
...
-(BOOL) changeClickState:(NSIndexPath*)indexPath clickState:(BOOL)clickState  ButtonIndex:(NSInteger)buttonIndex
{
    ...
    if (m_numGameCount == 10)
    {
        ...
        {
            BOOL isHave = FALSE;
            JZTableViewCell_DataBase* base = (JZTableViewCell_DataBase*)[[(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:section] tableHeaderArray] objectAtIndex:row];
    ...
    JZTableViewCell_DataBase* base = [[(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:section] tableHeaderArray] objectAtIndex:row];
    if (buttonIndex == 1)
    {
        if (clickState)
        {
    ...
    else if(buttonIndex == 5)
    {
        if (clickState)
        {
            [base setRQZQ_S_ButtonIsSelect:YES];
        }
        else
        {
            [base setRQZQ_S_ButtonIsSelect:NO];
    ...
    else if(buttonIndex == 6)
    {
        if (clickState)
        {
              [base setRQZQ_P_ButtonIsSelect:YES];
        }
        else
        {
            [base setRQZQ_P_ButtonIsSelect:NO];
    ...
    else if(buttonIndex == 7)
    {
        if (clickState)
        {
            [base setRQZQ_F_ButtonIsSelect:YES];
        }
        else
        {
            [base setRQZQ_F_ButtonIsSelect:NO];
    ...
    /*
     去除所选 胆
     */
    if (![base ZQ_S_ButtonIsSelect] && ![base ZQ_P_ButtonIsSelect] && ![base ZQ_F_ButtonIsSelect] && ![base RQZQ_S_ButtonIsSelect] && ![base RQZQ_P_ButtonIsSelect] && ![base RQZQ_F_ButtonIsSelect] && [[base sfc_selectTag] count] <= 0) {
        [base setJC_DanIsSelect:NO];
    }
    if (m_playTypeTag == IJCZQPlayType_RQ_SPF || m_playTypeTag == IJCZQPlayType_RQ_SPF_DanGuan || m_playTypeTag == IJCZQPlayType_SPF || m_playTypeTag == IJCZQPlayType_SPF_DanGuan || m_playTypeTag == IJCZQPlayType_RQANDNO_SPF) {
        //刷新 所选比赛场次，金额
        ...
        for (int i = 0; i < m_headerCount; i++)
        {
            int baseCount = [(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:i] baseCount];
            for (int j = 0; j < baseCount; j++) {
                BOOL ZQ_S_ButtonIsSelect = [(JZTableViewCell_DataBase*)[[(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:i] tableHeaderArray] objectAtIndex:j] ZQ_S_ButtonIsSelect];
                
                BOOL ZQ_P_ButtonIsSelect = [(JZTableViewCell_DataBase*)[[(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:i] tableHeaderArray] objectAtIndex:j] ZQ_P_ButtonIsSelect];
                
                BOOL ZQ_F_ButtonIsSelect = [(JZTableViewCell_DataBase*)[[(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:i] tableHeaderArray] objectAtIndex:j] ZQ_F_ButtonIsSelect];
                BOOL RQZQ_S_ButtonIsSelect = [(JZTableViewCell_DataBase*)[[(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:i] tableHeaderArray] objectAtIndex:j] RQZQ_S_ButtonIsSelect];
                
                BOOL RQZQ_P_ButtonIsSelect = [(JZTableViewCell_DataBase*)[[(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:i] tableHeaderArray] objectAtIndex:j] RQZQ_P_ButtonIsSelect];
                
                BOOL RQZQ_F_ButtonIsSelect = [(JZTableViewCell_DataBase*)[[(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:i] tableHeaderArray] objectAtIndex:j] RQZQ_F_ButtonIsSelect];
                ...
                if (m_playTypeTag == IJCZQPlayType_RQANDNO_SPF)
                {
                    if (RQZQ_S_ButtonIsSelect || RQZQ_P_ButtonIsSelect || RQZQ_F_ButtonIsSelect)
                    {
                        gameCount++;
                        if (RQZQ_S_ButtonIsSelect && RQZQ_P_ButtonIsSelect && !RQZQ_F_ButtonIsSelect)
                        {
                        ...
                        if (RQZQ_S_ButtonIsSelect && RQZQ_F_ButtonIsSelect && !RQZQ_P_ButtonIsSelect)
                        {
                            m_twoCount++;
                        }
                        if (RQZQ_F_ButtonIsSelect&& RQZQ_P_ButtonIsSelect && !RQZQ_S_ButtonIsSelect)
                        {
                            m_twoCount++;
                        }
    ...
    //选中的时候不希望在改变base的 base 的setJC_DanIsSelect该属性
    [m_jz_NoteNumEditeViewController.deleteBaseArray removeAllObjects];
...
-(void) reloadData
{
    ...
    m_numGameCount = gameCount;
    // 已选比赛：%d 场
    m_totalCost.text = [NSString stringWithFormat:@"%d 场", gameCount];
    if (gameCount>0)
    {
       [m_drawerView.countEventLable setHidden:NO];
        [m_drawerView.countUIImageView setHidden:NO];
        m_drawerView.countEventLable.text = [NSString stringWithFormat:@"%d",gameCount];
    }else
    {
        [m_drawerView.countEventLable setHidden:YES];
        [m_drawerView.countUIImageView setHidden:YES];
...
- (NSString*)getSfcScore:(NSInteger)section Row:(NSInteger)row Tag:(NSInteger)tag isGetBetCode:(BOOL)isGetbetCode
{
        ...
        m_playTypeTag == IJCZQPlayType_Score_DanGuan)
    {
        ...
        if (tag <= 12) {
            m_tempStr = [[(JZTableViewCell_DataBase*)[[(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:section] tableHeaderArray] objectAtIndex:row] score_S_Array] objectAtIndex:tag];
            
            temp = [temp stringByAppendingString:m_tempStr];
        }
        if( 12 < tag && tag < 18)
        {
            m_tempStr = [[(JZTableViewCell_DataBase*)[[(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:section] tableHeaderArray] objectAtIndex:row] score_P_Array] objectAtIndex:tag - 13];
            
            temp = [temp stringByAppendingString:m_tempStr];
        }
        if(tag >= 18)
        {
            m_tempStr = [[(JZTableViewCell_DataBase*)[[(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:section] tableHeaderArray] objectAtIndex:row] score_F_Array] objectAtIndex:tag - 18];
        ...
        m_playTypeTag == IJCZQPlayType_HalfAndAll_DanGuan)
    {
        ...
        m_tempStr = [[(JZTableViewCell_DataBase*)[[(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:section] tableHeaderArray] objectAtIndex:row] half_Array] objectAtIndex:tag];
        temp = [temp stringByAppendingString:m_tempStr];
...
- (void)parserData
{
    ...
    for (int i = 0; i < nHeaderCount; i++)
    {
        ...
        JZTableViewCell_DataArray* currentBaseArray = [[JZTableViewCell_DataArray alloc] init];
        ...
        for (int j = 0; j < subArrayCount; j++)
        {
            ...
            JZTableViewCell_DataBase*  base = [[JZTableViewCell_DataBase alloc] init];
    ...
    for (int i = 0; i < m_headerCount; i++)
    {
        if (i == 0) {
            ...
            m_SectionN[i] = [(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:i] baseCount];
        }
        else
        {
            m_SectionN[i] = 0;
...
- (void) changeLeague
{
    ...
    NSLog(@"m_league_tableCell_DataArraym_league_tableCell_DataArray%@",m_league_tableCell_DataArray);
}
...
- (void)setPlayChooseView
{
    ...
    {
        NSArray *titleArray = [NSArray arrayWithObjects:@"胜平负",@"总进球数",@"半全场",@"比分",@"混合投注", nil];
        m_playChooseView = [[TopAnimationView alloc] initWithFrame:CGRectMake(0, 0, 320, ([titleArray count]/3+1)*40+30) andNormalTitles:titleArray];
        m_playChooseView.backgroundColor = [ColorUtils parseColorFromRGB:@"#fffbf3"];
        [m_playChooseView thatButtonSelect:1];
        
        m_playChooseView.delegate = self;
...
- (void)topAnimationView:(TopAnimationView *)topAnimationView didSelectItemAtIndex:(NSUInteger)index
{
    ...
    switch (index) {
        ...
        {
            NSLog(@"胜平负");
            self.playTypeTag= IJCZQPlayType_RQANDNO_SPF;
        }
            break;
        case 2:
        {
            self.playTypeTag= IJCZQPlayType_ZJQ;
            NSLog(@"总进球数");
        }
            break;
        case 3:
        {
            self.playTypeTag= IJCZQPlayType_HalfAndAll;
            NSLog(@"半全场");
        }
            break;
        case 4:
        {
            self.playTypeTag= IJCZQPlayType_Score;
            NSLog(@"比分");
        }
            break;
        case 5:
...
- (void)setupNavigationBar
{
    ...
    m_detailButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 24, 24)];
    [m_detailButton addTarget:self action: @selector(detailViewButtonClick:) forControlEvents:UIControlEventTouchUpInside];
    [m_detailButton setImage:[UIImage imageNamed:@"right_top_list_button_normal"] forState:UIControlStateNormal];
    [m_detailButton setImage:[UIImage imageNamed:@"right_top_list_button_click"] forState:UIControlStateHighlighted];
    //    detailButton.imageEdgeInsets = UIEdgeInsetsMake(10, 20, 10, 0);
    m_detailButton.showsTouchWhenHighlighted = TRUE;
    //    [self.navigationController.navigationBar addSubview:m_detailButton];
    self.navigationItem.rightBarButtonItem = [[[UIBarButtonItem alloc] initWithCustomView:m_detailButton] autorelease];
...
-(void) gotoFenxiView:(NSIndexPath*)indexPath
{
    ...
    event = [event stringByAppendingFormat:@"%@_" ,[(JZTableViewCell_DataBase*)[[(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:section] tableHeaderArray] objectAtIndex:row] dayTime]];
    event = [event stringByAppendingFormat:@"%@_" ,[(JZTableViewCell_DataBase*)[[(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:section] tableHeaderArray] objectAtIndex:row] weekld]];
    
    event = [event stringByAppendingString:[(JZTableViewCell_DataBase*)[[(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:section] tableHeaderArray] objectAtIndex:row] teamld]];
...
-(BOOL) gameIsSelect:(NSIndexPath*) indexPath
{
    ...
    JZTableViewCell_DataBase* tableCell = (JZTableViewCell_DataBase*)[[(JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:section] tableHeaderArray] objectAtIndex:row];
...
-(BOOL) judegmentDan
{
    ...
    for (int index = 0; index < m_headerCount; index++) {
        JZTableViewCell_DataArray* header =   (JZTableViewCell_DataArray*)[m_tableCell_DataArray objectAtIndex:index];
        for (int spindex = 0; spindex < [header baseCount]; spindex++)
        {
            JZTableViewCell_DataBase* base = [[header tableHeaderArray] objectAtIndex:spindex];
            if ([base JC_DanIsSelect]) {
                danCount++;
            }
        }
...
- (void)randomSPF
{
    ...
    m_numGameCount = 2;
    m_totalCost.text = @"2 场\n";
    
    
}

#pragma mark 获得一场比赛的Event
- (NSString*) getEventWithBase:(JZTableViewCell_DataBase*)base
{
    NSString*   event = @"";
    event = [event stringByAppendingString:@"1_"];//1 足球 0篮球
