//
//  QLC_PickNumberViewController.m
//  RuYiCai
//
...
@interface QLC_PickNumberViewController (internal)
...
@implementation QLC_PickNumberViewController
...
//    [MobClick event:@"FCTC_selectPage"];
    ...
    [self newThreadRun];
    [[RuYiCaiNetworkManager sharedManager]getLotteryInformation:@"1" lotno:kLotNoQLC];
...
//    [self setDetailView];
    ...
    [[RuYiCaiNetworkManager sharedManager] highFrequencyInquiry:kLotNoQLC];//获取期号
}
...
- (void)refreshLeftTime
{
	...
	else if(leftTime == 0)//防止过期彩种
	{
		...
		[[RuYiCaiNetworkManager sharedManager] showMessage:[NSString stringWithFormat:@"七乐彩 %@期时间已截止，投注时请确认您选择的期号！" , self.batchCode] withTitle:@"提示" buttonTitle:@"确定"];
//        [[RuYiCaiNetworkManager sharedManager] highFrequencyInquiry:kLotNoQLC];//获取期号
	}
	else //时间为负时，停止调用
	{
...
- (void)queryBetLotButtonClick:(id)sender
{
    ...
    [CommonRecordStatus commonRecordStatusManager].QueryBetlotNO = kLotNoQLC;
...
- (void)setupQueryLotBetViewController
{
    ...
    [viewController setSelectLotNo:kLotNoQLC];
...
- (void)setupSubViewsOfDirectGroup
{
    ...
    labelRedBall.text = @"   选号区（请选择7－15个号码）";
    ...
    [m_redBallViewDirect createBallArray:30 withPerLine:9 startValue:1 selectBallCount:7];
	[m_redBallViewDirect setBallType:RED_BALL];
	[m_redBallViewDirect setSelectMaxNum:15];
    ...
    [m_randomNumView_red createNumBallList:9 withPerLine:8 startValue:7 withFrame:CGRectMake(0, 0, 320, kLabelHeight + frameRedBall.size.height) andBallType:RANDOM_RED_BALL];
...
- (void)setupSubViewsOfDragGroup
{
    ...
    labelRedDanBall.text = @"   胆码区：（请选择1~6个）";
    ...
    [m_redDanBallViewDrag createBallArray:30 withPerLine:9 startValue:1 selectBallCount:1];
	[m_redDanBallViewDrag setBallType:RED_BALL];
	[m_redDanBallViewDrag setSelectMaxNum:6];
    ...
    [m_randomNumView_redDan createNumBallList:6 withPerLine:8 startValue:1 withFrame:CGRectMake(0, 0, 320, kLabelHeight + frameRedDanBall.size.height) andBallType:RANDOM_RED_BALL];
    ...
    labelRedTuoBall.text = @"   拖码区：（请选择2~20个）";
    ...
    [m_redTuoBallViewDrag createBallArray:30 withPerLine:9 startValue:1 selectBallCount:2];
...
- (void)submitLotNotification:(NSNotification*)notification
{
    ...
    {
        NSArray *danBalls = [m_redDanBallViewDrag selectedBallsArray];
        NSArray *tuoBalls = [m_redTuoBallViewDrag selectedBallsArray];
...
//        disBetCode = [disBetCode stringByAppendingFormat:@"\n"];
    ...
    [RuYiCaiLotDetail sharedObject].lotNo = kLotNoQLC;
...
- (void)updateBallState:(NSNotification *)notification
{
    ...
    if (kSegmentedDirect == self.segmentedView.segmentedIndex)
    {
        ...
        //注数 = zuhe(7,用户选中红球数量)
        m_numZhu = RYCChoose(7, nRedBalls);
...
//            totalStr = [NSString stringWithFormat:@" 您已选择了 %d 注，共 %d 元", m_numZhu, m_numCost];
    ...
    else if (kSegmentedDrag == self.segmentedView.segmentedIndex)
    {
        ...
        if ([obj isKindOfClass:[NSDictionary class]])
        {
            ...
            if(ballView == m_redDanBallViewDrag)
            {
                //用胆码做比较  去 拖码的数
        ...
        //金额 = 注数 * 倍数 *（2元）* 期数
        if (nRedDanBalls >= 1) {
            m_numZhu = RYCChoose(7 - nRedDanBalls, nRedTuoBalls);
 
        }
        else
            m_numZhu = 0;
 
        if ([[m_redDanBallViewDrag selectedBallsArray] count] + [[m_redTuoBallViewDrag selectedBallsArray] count] < 8
            ) {
            m_numZhu = 0;
        }
...
- (IBAction)addBasketClick:(id)sender
{
    ...
    {
        if ([[m_redDanBallViewDrag selectedBallsArray] count] + [[m_redTuoBallViewDrag selectedBallsArray] count] < 8) {
            [[RuYiCaiNetworkManager sharedManager] showMessage:@"胆码和拖码之和至少为8个！" withTitle:@"提示" buttonTitle:@"确定"];
            return;
        }
        if ([[m_redDanBallViewDrag selectedBallsArray] count] + [[m_redTuoBallViewDrag selectedBallsArray] count] > 26) {
            [[RuYiCaiNetworkManager sharedManager] showMessage:@"胆码和拖码之和最多为26个！" withTitle:@"提示" buttonTitle:@"确定"];
            return;
        }
    }
...
- (void)historyLotteryClick:(id)sender
{
    ...
    viewController.lotTitle = kLotTitleQLC;
    viewController.lotNo = kLotNoQLC;
    viewController.VRednumber = 30;
    viewController.VBluenumber = 30;
...
- (void)motionEnded:(UIEventSubtype)motion withEvent:(UIEvent *)event
{
    if (motion == UIEventSubtypeMotionShake && m_delegate.isStartYaoYiYao  && self.segmentedView.segmentedIndex != kSegmentedDrag)
    {
        ...
        {
            [m_redBallViewDirect randomBall:7];
            [self updateBallState:nil];
        }
