//
//  FC3D_PickNumberViewController.m
//  RuYiCai
//
...
#define kMissDataIndexZX    (0)
#define kMissDataIndexZXS   (1)
#define kMissDataIndexZ36   (2)
#define kMissDataIndexZ36H  (3)

@interface FC3D_PickNumberViewController (internal)
...
@implementation FC3D_PickNumberViewController
...
//    [MobClick event:@"FCTC_selectPage"];
    ...
    [self newThreadRun];
    [[RuYiCaiNetworkManager sharedManager]getLotteryInformation:@"1" lotno:kLotNoFC3D];
...
{    
    ...
    [[RuYiCaiNetworkManager sharedManager] highFrequencyInquiry:kLotNoFC3D];//获取期号
    ...
    [[RuYiCaiNetworkManager sharedManager] getLotMissdateWithLotno:kLotNoFC3D sellWay:@"F47103MV_ZX"];
...
- (void)refreshLeftTime
{
	...
	else if(leftTime == 0)//防止过期彩种
	{
		...
		[[RuYiCaiNetworkManager sharedManager] showMessage:[NSString stringWithFormat:@"福彩3D %@期时间已截止，投注时请确认您选择的期号！" , self.batchCode] withTitle:@"提示" buttonTitle:@"确定"];
//        [[RuYiCaiNetworkManager sharedManager] highFrequencyInquiry:kLotNoFC3D];//获取期号
	}
	else //时间为负时，停止调用
	{
...
- (void)getMissDateOK:(NSNotification*)notification
{
    ...
    if(kSegmentedDirect == self.segmentedView.segmentedIndex)
    {
        if(0 == m_directType)
        {
            [self.recoderYiLuoDateArr replaceObjectAtIndex:kMissDataIndexZX withObject:[parserDict objectForKey:@"result"]];
        }
        else
        {
            [self.recoderYiLuoDateArr replaceObjectAtIndex:kMissDataIndexZXS withObject:[parserDict objectForKey:@"result"]];
        }
    }
    else if(kSegmentedGroup3 == self.segmentedView.segmentedIndex)
    {
        if(1 == m_group3Type)//复式
        {
             [self.recoderYiLuoDateArr replaceObjectAtIndex:kMissDataIndexZ36 withObject:[parserDict objectForKey:@"result"]];
        }
        else if(2 == m_group3Type)//和值
        {
             [self.recoderYiLuoDateArr replaceObjectAtIndex:kMissDataIndexZ36H withObject:[parserDict objectForKey:@"result"]];
        }
    }
    else
    {
...
- (void)refreshMissView
{
    if(kSegmentedDirect == self.segmentedView.segmentedIndex)
    {
        if(0 == m_directType)
        {
            [m_ballView100Direct creatYiLuoViewWithData:[[self.recoderYiLuoDateArr objectAtIndex:kMissDataIndexZX] objectForKey:@"bai"] rowNumber:10];
            [m_ballView10Direct creatYiLuoViewWithData:[[self.recoderYiLuoDateArr objectAtIndex:kMissDataIndexZX] objectForKey:@"shi"] rowNumber:10];
            [m_ballView1Direct creatYiLuoViewWithData:[[self.recoderYiLuoDateArr objectAtIndex:kMissDataIndexZX] objectForKey:@"ge"] rowNumber:10];
        }
        else
        {
            [m_ballViewDirectSum creatYiLuoViewWithData:[[self.recoderYiLuoDateArr objectAtIndex:kMissDataIndexZXS] objectForKey:@"miss"] rowNumber:9];
        }
    }
    else if(kSegmentedGroup3 == self.segmentedView.segmentedIndex)
    {
        if(1 == m_group3Type)
        {
            [m_ballViewDoubleGroup3 creatYiLuoViewWithData:[[self.recoderYiLuoDateArr objectAtIndex:kMissDataIndexZ36] objectForKey:@"miss"] rowNumber:9];
        }
        else if(2 == m_group3Type)
        {
            [m_ballViewGroup3Sum creatYiLuoViewWithData:[[self.recoderYiLuoDateArr objectAtIndex:kMissDataIndexZ36H] objectForKey:@"miss"] rowNumber:9];
        }
    }
    else
    {
        if(0 == m_group6Type)
        {
            [m_ballViewGroup6 creatYiLuoViewWithData:[[self.recoderYiLuoDateArr objectAtIndex:kMissDataIndexZ36] objectForKey:@"miss"] rowNumber:9];
        }
        else
        {
            NSMutableArray*  group6SumMissArray = [[NSMutableArray alloc] init];//减去前两个和后两个
            for(int i = 2; i < [[[self.recoderYiLuoDateArr objectAtIndex:kMissDataIndexZ36H] objectForKey:@"miss"] count] - 2; i++)
            {
                [group6SumMissArray addObject:[[[self.recoderYiLuoDateArr objectAtIndex:kMissDataIndexZ36H] objectForKey:@"miss"] objectAtIndex:i]];
            }
            [m_ballViewGroup6Sum creatYiLuoViewWithData:group6SumMissArray rowNumber:9];
            [group6SumMissArray release];
        }
    }
}
...
- (void)getMissDateNet
{
    ...
    {
        case kSegmentedDirect:
        {
            if(0 == m_directType)
            {
                if([[[self.recoderYiLuoDateArr objectAtIndex:kMissDataIndexZX] allKeys] count] == 0)
                {
                    [RuYiCaiNetworkManager sharedManager].netLotType = NET_LOT_MISSDATE;
                    [[RuYiCaiNetworkManager sharedManager] getLotMissdateWithLotno:kLotNoFC3D sellWay:@"F47103MV_ZX"];
                }
            ...
            {
                if([[[self.recoderYiLuoDateArr objectAtIndex:kMissDataIndexZXS] allKeys] count] == 0)
                {
                    [RuYiCaiNetworkManager sharedManager].netLotType = NET_LOT_MISSDATE;
                    [[RuYiCaiNetworkManager sharedManager] getLotMissdateWithLotno:kLotNoFC3D sellWay:@"F47103MV_ZXHZ"];
                }
                else
                {
                    [self refreshMissView];
        ...
        case kSegmentedGroup3:
        {
            if(1 == m_group3Type)
            {
                if([[[self.recoderYiLuoDateArr objectAtIndex:kMissDataIndexZ36] allKeys] count] == 0)
                {
                    [RuYiCaiNetworkManager sharedManager].netLotType = NET_LOT_MISSDATE;
                    [[RuYiCaiNetworkManager sharedManager] getLotMissdateWithLotno:kLotNoFC3D sellWay:@"F47103MV_Z36"];
                }
                else
                {
                    [self refreshMissView];
            ...
            {
                if([[[self.recoderYiLuoDateArr objectAtIndex:kMissDataIndexZ36H] allKeys] count] == 0)
                {
                    [RuYiCaiNetworkManager sharedManager].netLotType = NET_LOT_MISSDATE;
                    [[RuYiCaiNetworkManager sharedManager] getLotMissdateWithLotno:kLotNoFC3D sellWay:@"F47103MV_Z36HZ"];
                }
                else
                {
                    [self refreshMissView];
...
- (void)updateLottery:(NSNotification*)notification
{
    ...
    {
        if(i !=2)
            redStr = [redStr stringByAppendingFormat:@"%d,", [[winNo substringWithRange:NSMakeRange(i * 2, 2)] intValue]];
        else
            redStr = [redStr stringByAppendingFormat:@"%d", [[winNo substringWithRange:NSMakeRange(i * 2, 2)] intValue]];
    }
...
- (void)queryBetLotButtonClick:(id)sender
{
    ...
    [CommonRecordStatus commonRecordStatusManager].QueryBetlotNO = kLotNoFC3D;
...
- (void)luckButtonClick:(id)sender
{
    ...
    //    十一运夺金
    //    广东快乐十分
    [pickNumberView randomPickerView:m_delegate.randomPickerView selectRowNum:2];
...
- (void)setupQueryLotBetViewController
{
    ...
    [viewController setSelectLotNo:kLotNoFC3D];
...
{    
    ...
    CGRect frame100Ball = CGRectMake((320 - (9 * kBallVerticalSpacing) - 10 * kSmallBallRectWidth) / 2, 
                                     kLabelHeight + 50, 
    ...
    [m_ballView100Direct setBallSize:SMALL_BALL];
    m_ballView100Direct.isHasYiLuo = YES;
    [m_ballView100Direct createBallArray:10 withPerLine:10 startValue:0 selectBallCount:1];
    ...
    [m_ballView10Direct setBallSize:SMALL_BALL];
    m_ballView10Direct.isHasYiLuo = YES;
    [m_ballView10Direct createBallArray:10 withPerLine:10 startValue:0 selectBallCount:1];
    ...
    m_ballViewDirectSum.isHasYiLuo = YES;
	...
	[m_ballViewDirectSum setSelectMaxNum:SMALL_BALL];
	m_ballViewDirectSum.view.frame = ballFrame;
	[self.scrollDirect addSubview:m_ballViewDirectSum.view];
    
    self.scrollDirect.contentSize = CGSizeMake(320, frame1Ball.origin.y + frame1Ball.size.height + kYiLuoHeight + 5);
...
//    [numLabel release];
    ...
    m_ballViewDoubleGroup3.isHasYiLuo = YES;
    ...
    m_ballViewGroup3Sum.isHasYiLuo = YES;
...
- (void)setupSubViewsOfGroup6
{
    ...
    m_ballViewGroup6.isHasYiLuo = YES;
...
//    [label release];
    ...
    m_ballViewGroup6Sum.isHasYiLuo = YES;
...
- (void)submitLotNotification:(NSNotification*)notification
{
    ...
    if (kSegmentedDirect == self.segmentedView.segmentedIndex)
    {
        if(0 == m_directType)
        {
            ...
            betCode = [betCode stringByAppendingFormat:@"%02d", n100Count];
            ...
            betCode = [betCode stringByAppendingFormat:@"%02d", n10Count];
            ...
            betCode = [betCode stringByAppendingFormat:@"%02d", n1Count];
    ...
    {
        //betCode = [betCode stringByAppendingFormat:@"00%02d", numBeishu];
        if (0 == m_group3Type)  //组3单式
        {
                betCode = [betCode stringByAppendingString:@"0101"];
				...
				if(nValue1 < nValue2)//需排序
				{ 
					betCode = [betCode stringByAppendingFormat:@"%02d%02d%02d", nValue1, nValue1, nValue2];					
				}
				else
				{
					betCode = [betCode stringByAppendingFormat:@"%02d%02d%02d", nValue2, nValue1, nValue1];
				}
                disBetCode = [disBetCode stringByAppendingFormat:@"%d,%d,%d", nValue1, nValue1, nValue2];
        }
        else if(1 == m_group3Type)//复式
        {
            betCode = [betCode stringByAppendingString:@"3101"];
            NSArray* doubleBalls = [m_ballViewDoubleGroup3 selectedBallsArray];
            int nCount = [doubleBalls count];
            betCode = [betCode stringByAppendingFormat:@"%02d", nCount];
            for (int i = 0; i < nCount; i++)
            {
        ...
        else//组三和值
        {
            ...
            betCode = [betCode stringByAppendingString:@"1101"];
    ...
    {
        if(0 == m_group6Type)  //组6直选
        {
            NSArray* group6Balls = [m_ballViewGroup6 selectedBallsArray];
            int nCount = [group6Balls count];
            if (nCount > 3)
            {
                betCode = [betCode stringByAppendingString:@"3201"];
                betCode = [betCode stringByAppendingFormat:@"%02d", nCount];
            }
            else
            {
                betCode = [betCode stringByAppendingString:@"0201"];
            }
        ...
        else//组六和值
        {
            NSArray* selectedBallsArray;
            int nCount = 0;
            betCode = [betCode stringByAppendingString:@"1201"];
