...
@interface CQSF_PickNumberViewController (internal)
...
@implementation CQSF_PickNumberViewController
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [[RuYiCaiNetworkManager sharedManager] highFrequencyInquiry:kLotNoCQSF];
...
- (void)queryBetLotButtonClick:(id)sender
{
    ...
    [CommonRecordStatus commonRecordStatusManager].QueryBetlotNO = kLotNoCQSF;
...
- (void)setupQueryLotBetViewController
{
    ...
    [viewController setSelectLotNo:kLotNoCQSF];
...
{	
	switch (m_playType)
	{
		...
		{	
            ...
            [self getMissdateForServersWithPalyType:@"F47109MV_S1"];
		...
		case kPlayTypeRed:
		{
            ...
            [self getMissdateForServersWithPalyType:@"F47109MV_H1"];
			break;
		}
		case kPlayTypeRX2:
		{
            ...
            [self getMissdateForServersWithPalyType:@"F47109MV_RX"];
			break;
		}
		case kPlayTypeRX3:
		{
		...
		case kPlayTypeX3Zhi:
		{
            ...
            [self getMissdateForServersWithPalyType:@"F47109MV_Q3"];
			break;
		}	
		default:
			break;
...
- (void)submitLotNotification:(NSNotification*)notification
{
    ...
    [RuYiCaiLotDetail sharedObject].lotNo = kLotNoCQSF;
...
- (void)playIntroButtonClick:(id)sender
{
    ...
    viewController.lotNo = kLotNoCQSF;
    [self.navigationController pushViewController:viewController animated:YES];
    [viewController release];
}
...
- (void)historyLotteryClick:(id)sender
{
    ...
    viewController.lotTitle = kLotTitleCQSF;
    viewController.lotNo = kLotNoCQSF;
...
- (void)refreshLeftTime
{
	...
	else if(leftTime == 0)//防止过期彩种
	{
		...
		[[RuYiCaiNetworkManager sharedManager] showMessage:[NSString stringWithFormat:@"重庆快乐十分 %@期时间已到，进入下一期" , self.batchCode] withTitle:@"提示" buttonTitle:@"确定"];
	    [[RuYiCaiNetworkManager sharedManager] highFrequencyInquiry:kLotNoCQSF];
        [[RuYiCaiNetworkManager sharedManager]getLotteryInformation:@"1" lotno:kLotNoCQSF];//上期开奖
...
//    [self.ballViewTuo creatYiLuoViewWithData:array rowNumber:9];
    ...
    [RuYiCaiNetworkManager sharedManager].netLotType = NET_LOT_MISSDATE;
    [[RuYiCaiNetworkManager sharedManager] getLotMissdateWithLotno:kLotNoCQSF sellWay:type];
