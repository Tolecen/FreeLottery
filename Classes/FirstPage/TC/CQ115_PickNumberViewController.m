...
@interface CQ115_PickNumberViewController (internal)
...
@implementation CQ115_PickNumberViewController
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [[RuYiCaiNetworkManager sharedManager]highFrequencyInquiry:kLotNoCQ115];
...
- (void)newThreadRun
{
    ...
    [[RuYiCaiNetworkManager sharedManager] getLotMissdateWithLotno:kLotNoCQ115 sellWay:@"T01016MV_RX"];
...
- (void)queryBetLotButtonClick:(id)sender
{
    ...
    [CommonRecordStatus commonRecordStatusManager].QueryBetlotNO = kLotNoCQ115;
...
- (void)setupQueryLotBetViewController
{
    ...
    [viewController setSelectLotNo:kLotNoCQ115];
...
- (void)submitLotNotification:(NSNotification*)notification
{
    ...
    [RuYiCaiLotDetail sharedObject].lotNo = kLotNoCQ115;
...
- (void)getMissDateNet
{
    if(kSegmentedDirect == self.segmentedView.segmentedIndex)
    {
        if (m_playType == kPlayTypezhi2 || m_playType == kPlayTypezhi3 || m_playType == kPlayType1)
        {
            ...
            {
                [RuYiCaiNetworkManager sharedManager].netLotType = NET_LOT_MISSDATE;
                [[RuYiCaiNetworkManager sharedManager] getLotMissdateWithLotno:kLotNoCQ115 sellWay:@"T01016MV_Q3"];
            }
            else
            {
                [self refreshMissView];
        ...
        else if (m_playType == kPlayTypezu2)
        {
            ...
            {
                [RuYiCaiNetworkManager sharedManager].netLotType = NET_LOT_MISSDATE;
                [[RuYiCaiNetworkManager sharedManager] getLotMissdateWithLotno:kLotNoCQ115 sellWay:@"T01016MV_Q2Z"];
            }
            else
            {
                [self refreshMissView];
        ...
        else if(m_playType == kPlayTypezu3)
        {
            ...
            {
                [RuYiCaiNetworkManager sharedManager].netLotType = NET_LOT_MISSDATE;
                [[RuYiCaiNetworkManager sharedManager] getLotMissdateWithLotno:kLotNoCQ115 sellWay:@"T01016MV_Q3Z"];
            }
            else
            {
                [self refreshMissView];
...
- (void)playIntroButtonClick:(id)sender
{
    ...
    viewController.lotNo = kLotNoCQ115;
    [self.navigationController pushViewController:viewController animated:YES];
    [viewController release];
}
...
- (void)historyLotteryClick:(id)sender
{
    ...
    viewController.lotTitle = kLotTitleCQ11X5;
    viewController.lotNo = kLotNoCQ115;
...
- (void)refreshLeftTime
{
	...
	else if(leftTime == 0)//防止过期彩种
	{
		...
		[[RuYiCaiNetworkManager sharedManager] showMessage:[NSString stringWithFormat:@"重庆11选5%@期时间已到，进入下一期" , self.batchCode] withTitle:@"提示" buttonTitle:@"确定"];
