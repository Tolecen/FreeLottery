//
//  X115_PickNumberViewController.m
//  RuYiCai
//
//  Created by haojie on 11-10-14.
...
@interface X115_PickNumberViewController (internal)
...
@implementation X115_PickNumberViewController
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [[RuYiCaiNetworkManager sharedManager]highFrequencyInquiry:kLotNo115];
...
//    self.scrollDirect.contentSize = CGSizeMake(320, 150);
...
- (void)newThreadRun
{
    ...
    [[RuYiCaiNetworkManager sharedManager] getLotMissdateWithLotno:@"T01010" sellWay:@"T01010MV_RX"];
...
- (void)queryBetLotButtonClick:(id)sender
{
    ...
    [CommonRecordStatus commonRecordStatusManager].QueryBetlotNO = kLotNo115;
...
- (void)luckButtonClick:(id)sender
{
    ...
    //    十一运夺金
    //    广东快乐十分
    [pickNumberView randomPickerView:m_delegate.randomPickerView selectRowNum:4];
...
- (void)setupQueryLotBetViewController
{
    ...
    [viewController setSelectLotNo:kLotNo115];
...
- (void)updateSubViewsOfDrag
{
    switch (drag_PlayType) {
        ...
        case kPlayType7:
            ...
            break;
        case kPlayType8:
            m_danLabel.text = @"胆码（选1-7个球,胆码加拖码个数大于8）：";
            m_tuoLabel.text = @"拖码（选2-10个球）：";
            [self.ballViewDan setSelectMaxNum:7];
            [self.ballViewTuo setSelectMaxNum:10];
            break;
        case 7:
            m_danLabel.text = @"胆码（只能选1个球）：";
    ...
    NSArray* arrayDes = kwinDescribtionArray_JXdantuo;
    NSArray* arrayMonney = kwinMonneyArray_JXdantuo;
    m_winDescribtionLable_dan.text = [arrayDes objectAtIndex:drag_PlayType];
    m_winMonneyLable_dan.text = [arrayMonney objectAtIndex:drag_PlayType];
}
...
- (void)playTypeSet
{
    ...
    {
        [m_delegate.randomPickerView presentModalView:m_delegate.randomPickerView.view setPickerType:DRAG11X5_TYPE];
        switch (drag_PlayType) 
        {
            case kPlayType2:
                [m_delegate.randomPickerView setPickerNum:kPlayType2 withMinNum:0 andMaxNum:0];
            ...
            case 7:
                ...
                break;
            case 8:
                [m_delegate.randomPickerView setPickerNum:8 withMinNum:0 andMaxNum:0];
                break;
            default:
                break;
        }
...
- (void)getMissDateNet
{
    if(kSegmentedDirect == self.segmentedView.segmentedIndex)
    {
        if (m_playType == kPlayTypezhi2 || m_playType == kPlayTypezhi3 || m_playType == kPlayType1)
        {
            ...
            {
                [RuYiCaiNetworkManager sharedManager].netLotType = NET_LOT_MISSDATE;
                [[RuYiCaiNetworkManager sharedManager] getLotMissdateWithLotno:@"T01010" sellWay:@"T01010MV_Q3"];
            }
            else
            {
                [self refreshMissView];
        ...
        else if (m_playType == kPlayTypezu2)
        {
            ...
            {
                 [RuYiCaiNetworkManager sharedManager].netLotType = NET_LOT_MISSDATE;
                [[RuYiCaiNetworkManager sharedManager] getLotMissdateWithLotno:@"T01010" sellWay:@"T01010MV_Q2Z"];
            }
            else
            {
                [self refreshMissView];
        ...
        else if(m_playType == kPlayTypezu3)
        {
            ...
            {
                 [RuYiCaiNetworkManager sharedManager].netLotType = NET_LOT_MISSDATE;
                [[RuYiCaiNetworkManager sharedManager] getLotMissdateWithLotno:@"T01010" sellWay:@"T01010MV_Q3Z"];
            }
            else
            {
                [self refreshMissView];
...
- (void)refreshLeftTime
{
	...
	else if(leftTime == 0)//防止过期彩种
	{
		...
		[[RuYiCaiNetworkManager sharedManager] showMessage:[NSString stringWithFormat:@"江西11选5 %@期时间已到，进入下一期" , self.batchCode] withTitle:@"提示" buttonTitle:@"确定"];
