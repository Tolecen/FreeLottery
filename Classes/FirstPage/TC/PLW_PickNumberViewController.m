//
//  PLW_PickNumberViewController.m
//  RuYiCai
//
...
@interface PLW_PickNumberViewController (internal)
...
@implementation PLW_PickNumberViewController
...
//    [MobClick event:@"FCTC_selectPage"];
	...
	m_delegate = (RuYiCaiAppDelegate *)[[UIApplication sharedApplication] delegate];
     ...
     [self newThreadRun];
    [[RuYiCaiNetworkManager sharedManager]getLotteryInformation:@"1" lotno:kLotNoPL5];
...
//    [self setDetailView];
    ...
    [[RuYiCaiNetworkManager sharedManager] highFrequencyInquiry:kLotNoPL5];//获取期号
}
...
- (void)refreshLeftTime
{
	...
	else if(leftTime == 0)//防止过期彩种
	{
		...
		[[RuYiCaiNetworkManager sharedManager] showMessage:[NSString stringWithFormat:@"排列五 %@期时间已截止，投注时请确认您选择的期号！" , self.batchCode] withTitle:@"提示" buttonTitle:@"确定"];
//        [[RuYiCaiNetworkManager sharedManager] highFrequencyInquiry:kLotNoPL5];//获取期号
...
- (void)queryBetLotButtonClick:(id)sender
{
    ...
    [CommonRecordStatus commonRecordStatusManager].QueryBetlotNO = kLotNoPL5;
...
- (void)setupQueryLotBetViewController
{
    ...
    [viewController setSelectLotNo:kLotNoPL5];
...
{
    //直选
	UILabel *label10000  = [[UILabel alloc] initWithFrame:CGRectMake(10, 0, 310, kLabelHeight)];
	label10000.text = @"万位区（至少选择1个）：";
	...
	CGRect frameBall10000 = CGRectMake((320 - (9 * kBallVerticalSpacing) - 10 * kSmallBallRectWidth) / 2, 
									   ...
									   (kSmallBallRectHeight + kBallLineSpacing) * 1);
	...
	[m_ballView10000 setSelectMaxNum:7];
	m_ballView10000.view.frame = frameBall10000;
	[self.scrollDirect addSubview:m_ballView10000.view];
	
	UILabel *m_label1000  = [[UILabel alloc] initWithFrame:CGRectMake(10, frameBall10000.origin.y+frameBall10000.size.height, 310, kLabelHeight)];
	m_label1000.text = @"千位区（至少选择1个）：";
	m_label1000.textAlignment = UITextAlignmentLeft;
	m_label1000.backgroundColor = [UIColor clearColor];
	m_label1000.font = [UIFont systemFontOfSize:kLabelFontSize];
	[self.scrollDirect addSubview:m_label1000];
	[m_label1000 release];
	...
	[m_ballView1000 setSelectMaxNum:7];
	m_ballView1000.view.frame = frameBall1000;
	[self.scrollDirect addSubview:m_ballView1000.view];
	
	UILabel *m_label100  = [[UILabel alloc] initWithFrame:CGRectMake(10, frameBall1000.origin.y+frameBall1000.size.height, 310, kLabelHeight)];
	m_label100.text = @"百位区（至少选择1个）：";
	m_label100.textAlignment = UITextAlignmentLeft;
	m_label100.backgroundColor = [UIColor clearColor];
	m_label100.font = [UIFont systemFontOfSize:kLabelFontSize];
	[self.scrollDirect addSubview:m_label100];
	[m_label100 release];
	
	CGRect frameBall100 = CGRectMake(frameBall10000.origin.x, 
									 frameBall1000.origin.y+frameBall1000.size.height + kLabelHeight, 
                                     frameBall1000.size.width, 
                                     frameBall1000.size.height);
	...
	[m_ballView100 setSelectMaxNum:7];
	m_ballView100.view.frame = frameBall100;
	[self.scrollDirect addSubview:m_ballView100.view];
	
	
    UILabel *m_label10  = [[UILabel alloc] initWithFrame:CGRectMake(10, frameBall100.origin.y+frameBall100.size.height, 310, kLabelHeight)];
	m_label10.text = @"十位区（至少选择1个）：";
	...
	[self.scrollDirect addSubview:m_label10];
	[m_label10 release];
	...
	[m_ballView10 setSelectMaxNum:7];
	m_ballView10.view.frame = frameBall10;
	[self.scrollDirect addSubview:m_ballView10.view];
	
	UILabel *m_label1 = [[UILabel alloc] initWithFrame:CGRectMake(10, frameBall10.origin.y+frameBall10.size.height, 310, kLabelHeight)];
	m_label1.text = @"个位区（至少选择1个）：";
	...
	[self.scrollDirect addSubview:m_label1];
	[m_label1 release];
...
- (void)submitLotNotification:(NSNotification*)notification
{
    ...
    [RuYiCaiLotDetail sharedObject].lotNo = kLotNoPL5;
...
- (void)historyLotteryClick:(id)sender
{
    ...
    viewController.lotTitle = kLotTitlePL5;
    viewController.lotNo = kLotNoPL5;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath 
{
    ...
    if (cell == nil)
    {
		...
		[cell setRedNum:5 inRedMax:10 dxds:NO];
